...
export function FluxoCaixa() {
  ...
  // ── Fluxo diário (dias do mês selecionado) ───────────────────────
  ...
  const diasFluxo = Array.from({ length: diasDoMes }, (_, i) => {
    ...
    const entradas  = vendasDia.reduce((s, v) => s + v.totalFinal, 0);
    const descontos = vendasDia.reduce((s, v) => s + v.desconto, 0);
    return {
      dia,
  ...
  // ── Fluxo mensal (12 meses do ano) ───────────────────────────────
  const mesesFluxo = Array.from({ length: 12 }, (_, m) => {
    const vendasMes = vendas.filter(v => {
      const d = new Date(v.criadaEm);
      return d.getFullYear() === anoRef && d.getMonth() === m;
    });
    const entradas  = vendasMes.reduce((s, v) => s + v.totalFinal, 0);
    const descontos = vendasMes.reduce((s, v) => s + v.desconto, 0);
    return { mes: m, label: MESES[m].slice(0, 3), entradas, descontos, qtdVendas: vendasMes.length };
  });
    ...
    <div className="page">
      ...
      {aba === 'diario' && (
        <>
          ...
          <div className="fc-nav">
            <button className="btn-secondary fc-nav-btn" onClick={() => navMes(-1)}>
              <ChevronLeft size={16} />
            </button>
          ...
          <div className="fc-stats">
            ...
            <div className="stat-card">
              <div className="stat-label"><DollarSign size={12} style={{ verticalAlign: -1 }} /> Média por dia com venda</div>
              <div className="stat-value" style={{ fontSize: 20 }}>{fmt(mediaVendaDia)}</div>
              <div className="stat-sub">{diasComVenda} dia(s) com vendas</div>
            </div>
            <div className="stat-card">
              <div className="stat-label"><TrendingDown size={12} style={{ verticalAlign: -1 }} /> Descontos concedidos</div>
              ...
              {diasFluxo.map(d => (
                <div key={d.label} className={`fc-col${d.isHoje ? ' hoje' : ''}${d.isFuturo ? ' futuro' : ''}`}>
                  <div className="fc-col-valor">
                    {d.entradas > 0 && (
                      <span className="fc-col-tip">{fmt(d.entradas)}</span>
                    )}
                    <div
                      className="fc-bar"
              ...
              <table>
                ...
                <tbody>
                      ...
                      <td style={{ width: 120 }}>
                        <div style={{ height: 6, background: 'var(--bg-3)', borderRadius: 3, overflow: 'hidden' }}>
                          <div style={{
                            ...
                            width: `${(d.entradas / maxDia) * 100}%`, opacity: 0.8,
                          }} />
                        </div>
                      </td>
                    </tr>
                  ...
                  {diasFluxo.filter(d => d.qtdVendas > 0).length === 0 && (
                    <tr><td colSpan={5} style={{ textAlign: 'center', color: 'var(--text-3)', padding: '32px 0' }}>
      ...
      {aba === 'mensal' && (
        <>
          {/* Navegação do ano */}
          <div className="fc-nav">
            <button className="btn-secondary fc-nav-btn" onClick={() => navAno(-1)}>
              <ChevronLeft size={16} />
            </button>
          ...
          <div className="fc-stats">
            ...
            <div className="stat-card">
              ...
              <div className="stat-sub">
                {fmt(Math.max(...mesesFluxo.map(m => m.entradas)))}
              </div>
            </div>
            <div className="stat-card">
              ...
              {mesesFluxo.map(m => {
                ...
                return (
                  <div key={m.mes} className={`fc-col${isMesAtual ? ' hoje' : ''}${isFuturo ? ' futuro' : ''}`}>
                    <div className="fc-col-valor">
                      {m.entradas > 0 && (
                        <span className="fc-col-tip">{fmt(m.entradas)}</span>
                      )}
                      <div
                        className="fc-bar"
                      ...
                      <tr key={m.mes} style={isMesAtual ? { background: 'var(--accent-bg)' } : {}}>
                        ...
                        <td style={{ width: 120 }}>
                          <div style={{ height: 6, background: 'var(--bg-3)', borderRadius: 3, overflow: 'hidden' }}>
                            <div style={{
                              height: '100%', borderRadius: 3, background: 'var(--green)',
                              width: `${(m.entradas / maxMes) * 100}%`, opacity: 0.8,
                            }} />
                          </div>
                        </td>
                      </tr>
