...
export function Relatorios() {
  ...
  const rankProdutos = produtos.map(p => {
    const itensVend = vendasFiltradas.flatMap(v => v.itens).filter(i => i.produtoId === p.id);
    const qtd       = itensVend.reduce((s, i) => s + i.quantidade, 0);
    const receita   = itensVend.reduce((s, i) => s + i.subtotal, 0);
    const lucro     = receita - qtd * p.precoCusto;
    return { ...p, qtdVendida: qtd, receita, lucro };
  })
  .filter(p => p.qtdVendida > 0)
  .sort((a, b) => b.qtdVendida - a.qtdVendida);

  const maxQtd = rankProdutos[0]?.qtdVendida || 1;
  ...
  const porCategoria = ['semi-joias', 'maquiagem', 'acessorios', 'outro'].map(cat => {
  ...
  const labelCat: Record<string, string> = {
    'semi-joias': 'Semi Joias', 'maquiagem': 'Maquiagem',
    'acessorios': 'Acessórios', 'outro': 'Outro',
  };
      ...
      <div className="page-header">
        ...
        <div className="rel-periodo-tabs">
          {PERIODOS.map(p => (
            <button
              key={p.value}
              className={`cat-tab${periodo === p.value ? ' active' : ''}`}
      ...
      {vendasFiltradas.length === 0 ? (
        ...
        <div className="rel-grid">
          ...
          <div className="card rel-card">
            ...
            ) : rankProdutos.slice(0, 10).map((p, i) => (
              <div key={p.id} className="rel-bar-row">
                <div className="rel-rank">#{i + 1}</div>
                <div className="rel-bar-info">
                  <div className="rel-bar-label">
                    <span className="rel-nome">{p.nome}</span>
                    <span className="rel-valor">{p.qtdVendida} un. · {fmt(p.receita)}</span>
                  </div>
                  <div className="rel-bar-wrap">
                    <div
                      className="rel-bar rel-bar-accent"
                      style={{ width: `${(p.qtdVendida / maxQtd) * 100}%` }}
                    />
                  </div>
                  <div className="rel-lucro">
                    Lucro estimado: <strong style={{ color: 'var(--green)' }}>{fmt(p.lucro)}</strong>
                  </div>
                </div>
              </div>
            ))}
            ...
            <div className="card rel-card">
              ...
              {porCategoria.map(c => (
                <div key={c.categoria} className="rel-bar-row" style={{ marginBottom: 14 }}>
                  <div className="rel-bar-info">
                    <div className="rel-bar-label">
                      <span className="rel-nome">{labelCat[c.categoria]}</span>
                      <span className="rel-valor">{fmt(c.receita)}</span>
                    </div>
                    <div className="rel-bar-wrap">
                      <div
                        className="rel-bar rel-bar-blue"
                    ...
                    {[...vendasFiltradas]
                      ...
                      .map(v => (
                        <tr key={v.id}>
                          ...
                          <td style={{ fontSize: 12 }}>{v.nomeCliente || <span style={{ color: 'var(--text-3)' }}>—</span>}</td>
