...
export function Produtos() {
  ...
  const [usarCodigoPorVariacao, setUsarCodigoPorVariacao] = useState(false);
  ...
  function abrirNovo() {
    ...
    setVariacoes([]);
    setUsarCodigoPorVariacao(false);
    setEditId(null);
    setModal('novo');
  }
  ...
  function abrirEditar(p: Produto) {
    ...
    api.get<any[]>(`/api/produtos/${p.id}/variacoes`).then(res => {
      const lista = res.map((v: any) => ({
        id: v.id, tamanho: v.tamanho, cor: v.cor,
        outroCampo: v.outroCampo, codigoBarras: v.codigoBarras, estoque: v.estoque,
        estoqueMinimo: v.estoqueMinimo, ativo: v.ativo,
      }));
      setVariacoes(lista);
      setUsarCodigoPorVariacao(lista.some((v: Variacao) => !!v.codigoBarras));
    }).catch(() => setVariacoes([]));
  }
          ...
          <div className="modal" style={{ maxWidth: 800 }}>
            ...
            <div className="modal-body">
              <div className="form-grid">
                ...
                {/* Variações */}
                {!catSemGrade && (
                  <div className="variacoes-secao" style={{ gridColumn: '1/-1' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, paddingTop: 14, borderTop: '1px solid var(--border)' }}>
                      ...
                      <button className="btn-secondary" style={{ padding: '4px 10px', fontSize: 12 }}
                        onClick={() => setVariacoes(prev => [...prev, { tamanho: '', cor: '', codigoBarras: '', estoque: 0, estoqueMinimo: 1 }])}>
                        + Adicionar
                      </button>
                    </div>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, cursor: 'pointer', marginBottom: 10 }}>
                      <input type="checkbox" checked={usarCodigoPorVariacao} style={{ width: 16, height: 16, margin: 0 }}
                        onChange={e => setUsarCodigoPorVariacao(e.target.checked)} />
                      <span>Cada variação tem código de barras próprio</span>
                    </label>
                    {variacoes.length === 0 ? (
                      ...
                      <div className="variacoes-scroll-wrap" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        <div className="variacao-header-row" style={{ display: 'grid', gridTemplateColumns: `${[catUsaTamanho && '110px', catUsaCor && '110px', usarCodigoPorVariacao && '220px'].filter(Boolean).join(' ')} 60px 55px 28px`, gap: 6 }}>                        
                        ...
                        {variacoes.map((v, i) => (
                          <div key={i} className="variacao-row" style={{ display: 'grid', gridTemplateColumns: `${[catUsaTamanho && '110px', catUsaCor && '110px', usarCodigoPorVariacao && '220px'].filter(Boolean).join(' ')} 60px 55px 28px`, gap: 6, alignItems: 'center' }}>                            {catUsaTamanho && (
